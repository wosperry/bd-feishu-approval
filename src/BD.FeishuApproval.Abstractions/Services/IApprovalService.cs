...
public interface IApprovalService
{
    ...
    Task<CreateInstanceResult> CreateApprovalAsync<T>(T request) 
    ...
    /// <param name="callbackEvent">回调事件数据</param>
    /// <returns>处理任务</returns>
    Task HandleApprovalCallbackAsync(string approvalType, FeishuCallbackEvent callbackEvent);
    ...
    bool IsApprovalTypeSupported(string approvalType);
    ...
    /// <typeparam name="T">审批请求DTO类型</typeparam>
    /// <returns>是否已注册</returns>
    bool IsApprovalTypeSupported<T>() where T : class, IFeishuApprovalRequest, new();
    
    /// <summary>
    /// 获取所有支持的审批类型
    /// </summary>
    /// <returns>审批类型列表</returns>
    string[] GetSupportedApprovalTypes();
}